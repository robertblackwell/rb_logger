#ifndef trog_sample_configure_trog_hpp
#define trog_sample_configure_trog_hpp
...
///
///  The Trog logger is a typedef/using that renames an instantiation of a templates class.
///
...
namespace Trog {
    ...
    using Trogger = Collector<Formatter, SinkDefault, Simple::Writer>;
    /// leave these function definitions as they are
    inline void set_level(Trog::LogLevelType level)
    {
        Trogger::getInstance().set_threshold(level);
    }
    inline LogLevelType  get_level()
    {
        return Trogger::getInstance().get_threshold();
    }
    /// Formatter
    /// =========
    ///
